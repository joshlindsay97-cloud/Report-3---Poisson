...
- visits[n,n] : total interior visit counts (all walkers)
- exits[n,n]  : boundary exit counts (all walkers)
- g_charge    : h^2 * visits / Nwalkers
- g_exit_prob : exits / Nwalkers  (nonzero only at boundary)
- g_exit_sig  : sqrt(p(1-p)/N) per boundary site (binomial approx)
